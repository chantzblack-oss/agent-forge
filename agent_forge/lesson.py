...
_LESSON_VIDEO_SYSTEM = (
    ...
    "photo (OPTIONAL Wikimedia search query for a REAL photograph when "
    "an actual person/place/machine beats a diagram — specific proper "
    "nouns; 2-4 scenes per video), "
    "image (a cinematic shot description — like a film still: subject, "
    "angle, lighting — for scenes where neither photo nor chart fits; "
    "the engine paints it as the scene's full-bleed art; most scenes "
    "should carry photo, image, or data), "
...
def build_lesson(topic: str, on_progress=None, on_doc=None,
    ...
    vid = _video.build_video(
        ...
        mood="bright", audio=audio, checkpoint=checkpoint,
        clips_dir=clips_dir
    )
    return {"title": title, "doc": doc_path, "video": vid["path"],
            "voiced": vid["voiced"], "scenes": vid["scenes"],
            "quality": vid.get("quality"), "minutes": vid.get("minutes"),
            "fallback": vid.get("fallback")}
